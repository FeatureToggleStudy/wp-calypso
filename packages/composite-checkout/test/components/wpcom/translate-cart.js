 ...
 */
import { translateWpcomCartToCheckoutCart } from '../../../src/components/wpcom/translate-cart';
...
describe( 'translateWpcomCartToCheckoutCart', function() {
	describe( 'Cart with one plan only (BRL)', function() {
		...
		describe( 'second cart item (taxes)', function() {
			it( 'has the expected label', function() {
				expect( clientCart.items[ 1 ].label ).toBe( 'Tax' );
			} );
			it( 'has the expected type', function() {
				expect( clientCart.items[ 1 ].type ).toBe( 'tax' );
			} );
			it( 'has the expected currency', function() {
				expect( clientCart.items[ 1 ].amount.currency ).toBe( 'BRL' );
			} );
			it( 'has the expected value', function() {
				expect( clientCart.items[ 1 ].amount.value ).toBe( 500 );
			} );
			it( 'has the expected display value', function() {
				expect( clientCart.items[ 1 ].amount.displayValue ).toBe( 'R$5' );
			} );
		} );
	...
	describe( 'Cart with one plan and one bundled domain (BRL)', function() {
		...
		describe( 'third cart item (taxes)', function() {
			it( 'has the expected label', function() {
				expect( clientCart.items[ 2 ].label ).toBe( 'Tax' );
			} );
			it( 'has the expected type', function() {
				expect( clientCart.items[ 2 ].type ).toBe( 'tax' );
			} );
			it( 'has the expected currency', function() {
				expect( clientCart.items[ 2 ].amount.currency ).toBe( 'BRL' );
			} );
			it( 'has the expected value', function() {
				expect( clientCart.items[ 2 ].amount.value ).toBe( 500 );
			} );
			it( 'has the expected display value', function() {
				expect( clientCart.items[ 2 ].amount.displayValue ).toBe( 'R$5' );
			} );
		} );
